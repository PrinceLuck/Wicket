...
obj.getPaths().getAt(i);verts=[];for(j=0;j<obj.getPaths().getAt(i).length;j+=1)verts.push({x:tmp.getAt(j).lng(),y:tmp.getAt(j).lat()});if(!tmp.getAt(tmp.length-1).equals(tmp.getAt(0)))verts.push({x:tmp.getAt(0).lng(),y:tmp.getAt(0).lat()});rings.push(verts)}return{type:"polygon",components:rings}}if(obj.constructor===google.maps.Rectangle){tmp=obj.getBounds();return{type:"polygon",isRectangle:true,components:[[{x:tmp.getSouthWest().lng(),y:tmp.getNorthEast().lat()},{x:tmp.getNorthEast().lng(),y:tmp.getNorthEast().lat()},
{x:tmp.getNorthEast().lng(),y:tmp.getSouthWest().lat()},{x:tmp.getSouthWest().lng(),y:tmp.getSouthWest().lat()},{x:tmp.getSouthWest().lng(),y:tmp.getNorthEast().lat()}]]}}if(Wkt.isArray(obj)){features=[];for(i=0;i<obj.length;i+=1)features.push(this.deconstruct.call(this,obj[i]));return{type:function(){var k,type=obj[0].constructor;for(k=0;k<obj.length;k+=1)if(obj[k].constructor!==type)return"geometrycollection";switch(type){case google.maps.Marker:return"multipoint";case google.maps.Polyline:return"multilinestring";
case google.maps.Polygon:return"multipolygon";default:return"geometrycollection"}}(),components:function(){var i,comps;comps=[];for(i=0;i<features.length;i+=1)if(features[i].components)comps.push(features[i].components);return comps}()}}if(obj.getBounds&&obj.getRadius)console.log("Deconstruction of google.maps.Circle objects is not yet supported");else console.log("The passed object does not have any recognizable properties.")};
